...
WITH (
  external_location = 's3a://adam-data-client1/adam/full/{table_name}/',
  format            = 'JSON'
)
AS
SELECT
...
SELECT * FROM {CATALOGO_ORIGEM}.{SCHEMA_ORIGEM}.{tabela}
"""
        )
...
############################################
    for join in tabelas_com_join:
        ...
        insert = create_trino_task(
            ...
            sql=f"""INSERT INTO {CATALOGO_DESTINO}.{SCHEMA_DESTINO}.{t1}
                    SELECT T.* FROM {CATALOGO_ORIGEM}.{SCHEMA_ORIGEM}.{t1} T
...
############################################
    for join in tabelas_com_dois_joins:
        ...
        insert = create_trino_task(
            ...
            sql=f"""INSERT INTO {CATALOGO_DESTINO}.{SCHEMA_DESTINO}.{t1}
                    SELECT T1.* FROM {CATALOGO_ORIGEM}.{SCHEMA_ORIGEM}.{t1} T1
                    INNER JOIN {CATALOGO_ORIGEM}.{SCHEMA_ORIGEM}.{t2} T2
                        ON T1.{c1} = T2.{c2}
                    INNER JOIN {CATALOGO_ORIGEM}.{SCHEMA_ORIGEM}.{t3} T3
                        ON T2.{c3} = T3.{c4}
    ...
    for join in tabelas_com_tres_joins:
        ...
        insert = create_trino_task(
            ...
            sql=f"""INSERT INTO {CATALOGO_DESTINO}.{SCHEMA_DESTINO}.{t1}
                    ...
                    INNER JOIN {CATALOGO_ORIGEM}.{SCHEMA_ORIGEM}.{t3} T3
                        ON T2.{c3} = T3.{c4}
                    INNER JOIN {CATALOGO_ORIGEM}.{SCHEMA_ORIGEM}.{t4} T4
                        ON T3.{c5} = T4.{c6}
...
with DAG(
    "client1_lakehouse_json",
    description="ETL Dinâmico: CTAS em JSON e INSERTs com filtros e joins",
    default_args=default_args,
    schedule_interval='@once',
    catchup=False,
    concurrency=2,
    max_active_runs=1,
    tags=["trino", "lakehouse", "client1"]
) as dag:
